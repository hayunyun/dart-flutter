...
class _MainPageState extends State<MainPage> {
  ...
  ListView alarmPage(){
    return ListView.builder(
      padding:const Edge Insets.all(8),
...
void startAlarm() async{
  ...
  SendPort sendPort = IsolatedNameServer.lookupPortByName('rebirth');
  sendPort.send('start');
...
void stopAlarm(){
    print('stopAlarm()');
    SendPort sendPort = IsolatedNameServer.lookupPortByName('player');
    if(sendPort != null){
      sendPort.send('stop');
    }
  }
...
class AlarmTile extends StatefulWidget {
  ...
  AlarmTile(this._alarmInfo, thils._index);
...
class AlarmTileState extends State<AlarmTile>{
  ...
  Widge build(BuildContext context){
    ...
    _isChecked = widget._alarmInfo[widget._index]._isAlarmOn();
    ...
    return ListTile(
      ...
      trailing: Switch(
        ...
        onChanged: (value) async{
          ...
          .setAlarmOn(!widget._alarmInfo[widget._index]._isAlarmOn());
          var alarmID = await getAlarmID(widget._index);
          var alarmID = widget._index;
          if(widget._alarmInfo[widget._index]._isAlarmOn()){
